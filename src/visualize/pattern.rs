//! AST Visualizer for the pattern IR.

use crate::analysis::inference::{Type, TypeDisplay, TypeScheme};
use crate::analysis::name_table::NameTable;
use crate::ir::constructor::{Pattern, PatternKind};
use crate::ir::pattern::{Cluster, Crate, Def, Expr, ExprKind, MatchBranch};
use std::fmt::Write;
use std::rc::Rc;
...
pub struct PatternVisualizer<'a> {
    name_table: &'a NameTable,
    indent: usize,
    output: String,
}

impl<'a> PatternVisualizer<'a> {
    ...
    fn visit_match_branch(&mut self, b: &MatchBranch) {
        ...
        self.indent -= 1;
        if let Some(scheme) = &b.scrutinee_scheme {
            let scheme_s = scheme_str(scheme, self.name_table);
            self.write_line(&format!("scrutinee_scheme: {}", scheme_s));
        }
        self.write_line("body:");
        self.indent += 1;
...
pub fn visualize_crate(c: &Crate, name_table: &NameTable) -> String {
    let mut v = PatternVisualizer::new(name_table);
    v.visualize_crate(c)
}
