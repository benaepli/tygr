...
pub fn find_and_verify_main(
...
) -> Result<GlobalName, MainFunctionError> {
    ...
    if let TypeKind::Function(arg, _arg) = &def.ty.as_ref().ty {
        let is_unit = |t: &Rc<crate::analysis::inference::Type>| matches!(t.as_ref().ty, TypeKind::Con(id) if id.krate == None && id.name == crate::builtin::UNIT_TYPE);
